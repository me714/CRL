...
class ConvNet(nn.Module):
    def __init__(self, depth, flatten = False):
        ...
        for i in range(depth):
            ...
            outdim = 64
            B = ConvBlock(indim, outdim, pool = ( i in [0,1] ), padding = 0 if i in[0,1] else 1  ) #only pooling for fist 4 layers
            trunk.append(B)
...
class ConvNetNopool(nn.Module): #Relation net use a 4 layer conv with pooling in only first two layers, else no pooling
    def __init__(self, depth, flatten = True):
        ...
        if flatten:
            ...
            trunk.append(lastpool)
            trunk.append(Flatten())
            self.final_feat_dim = 15
        else:
            lastpool = nn.AdaptiveAvgPool2d((7, 7))
            trunk.append(lastpool)
            self.final_feat_dim = [15, 7, 7]
...
class ResNet(nn.Module):
    ...
    def __init__(self,block,list_of_num_layers, list_of_out_dims, flatten = True):
        ...
        if flatten:
            conv1x1 = nn.Conv2d(512, 24, kernel_size=1,)
            trunk.append(conv1x1)
            trunk.append(Flatten())
